...
START:
	...
	mov ax, 0x000	; 스택 세그먼트의 시작 어드레스(0x0000)를 세그먼트 레지스터 값으로 변환
...
.SCREENCLEARLOOP:					; 화면을 지우는 루프
	mov byte [ es: si ], 0			; 비디오 메모리의 문자가 위치하는 어드레스에
									; 0을 복사하여 문자를 삭제
	mov byte [ es: si + 1 ], 0x07	; 비디오 메모리의 속성이 위치하는 어드레스에
   									; 0x07(검은 바탕에 흰색)을 복사 ; 0x0A(검은 바탕에 밝은 녹색)을 복사
...
RESETDISK:							; 디스크를 리셋하는 코드의 시작
	...
	mov bx, 0x0000					; BX 레지스터에 0x0000을 설정하여 복사함
									; 어드레스를 0x1000:0000(0x100000)으로 최종 설정
...
READDATA:							; 디스크를 읽는 코드의 시작
	...
	sub di, 0x13
	add di, word [ SECTORNUMBER ]   ; 복사할 섹터 수를 17(처음) 혹은 18 감소
	...
	mov al, 0x13					;
	sub al, byte [ SECTORNUMBER ]	; 읽을 섹터 수는 17 혹은 18
	...
	;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
	mov ax, 0x13
	sub ax, word [ SECTORNUMBER ]
	mov cx, 0x0020
	mul cx
	add si, cx						; 512(0x200)바이트 x (읽은 섹터의 수(17 or 18)을 곱한 값
...
PRINTMESSAGE:
	...
	mov ax, 0xB800					; 비디오 메모리 시작 어드레스(0xB8000)를
									; 세그먼트 레지스터 값으로 변환
	mov es, ax						; ES 세그먼트 레지스터에 설정
	...
	mov si, 2						; 한 문자를 나타나는 바이트 수(2)를 SI 레지스터에 설정
